...
字符串(str)：是一个有序容器类数据，有索引，与列表相同
1.字符串定义：空列表([], list(), 相当于False)
string = "" 相当于False
s = str()   定义空字符串
2.字符串取值，同列表相似，也是使用索引，字符串名[索引]
3.字符串切片，和列表相似，左闭右开，右边取不到，起始位置不写默认为0索引，结束位置不写默认为字符串长度 步长：默认是1
4.字符串相加，拼接字符串
5.字符串乘数
6.判断字符串中是否有某个字符串
7.index():获取子字符串的首字符索引，有多个重复取第一个  可以有起始位置，终止位置（取不到）；当找不到时会报错
8.find():与index()用法相同，找不到不会报错，会返回-1
9.count():计算子字符串中出现的个数
10.spilt():分割字符串，返回一个列表，当不填写参数，默认分割空格，\t,\n等...可以使用符号进行分割，字符都行，分割后，用于分隔的字符没有了，每个的每部分都放入列表中，可以指定分割的最大数
11.strip():默认去掉首位空格，可以去掉字符串首尾的字符
12.replace():替代  注意：字符串中的元素不能进行修改
13.upper():将小写替换成大写
14.lower()：将大写转换成小写
15.join():拼接字符串，将序列拼接如字符串，这个字符串会将序列的每个元素进行分割
16.format():拼接字符串
   拼接字符串
   拼接浮点数
   4f：保留浮点数后四位
   ...
   调位置
   判断字符串
17.isaloha():判断是否为纯字母 是纯字母结果为True， 否则为False
