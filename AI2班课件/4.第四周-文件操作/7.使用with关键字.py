...
with open(file="水调歌头.txt",mode="r",encoding="utf-8")as f:
    ...
    print(f.read())
"""
# witn open(file="文件名", mode="模式", encoding="uft-8") as 变量:
    代码块 # 代码块需要缩进，只有在代码块中，才能进行 文件操作，出啦代码块范围，文件就关闭啦
"""
...
# f.seek(0)# 这里文件已经关闭
"""
练习：学号，姓名，性别，班级，成绩 把这个当成表头写入csv文件，使用键盘输入添加数据
判断学号是否纯数字重复，性别是否为男女，成绩0-100，筛选出不及格的学生，写入新的csv文件，补考.csv
"""
with open(file="学生信息.csv", mode="a+", encoding="utf-8") as fs:
    # 判断第一行内容
    fs.seek(0)
    ...
    # 处理字符串 stu_info;split() 指定字符串，返回列表
