"""
文件：命名：文件名字.拓展名 例如：写python .py文件, .txt普通文件, .doc(word文档), .csv, .xlsx, .ppt   windows系统会根据文件名字的后缀来确定当前文件类型
文本文件：写入的给用户看，以ASCII码存储的文件
二进制文件：给计算看
ASCII:使用八位二进制，对键盘字母，数字，符号进行了编码，转换二进制，
utf-8: 编码字符集，国际统一，对各国的文字进行编码，# -*- coding: utf-8 -*- 指定当前py文件为utf-8
GBK/gb2312:国标
file:是一个python语言的一个对象，可以通过open()打开文件进行操作
"""
...
f = open(filer=r"静夜思.txt", encoding="utf-8")
# read():方法，属于文档对象，读取文件中所有数据，返回一个长字符串
string = f.read()
print(string)
